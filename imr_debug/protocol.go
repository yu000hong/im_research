...
func WriteMessage(buf *bytes.Buffer, msg *Message) {
	body := msg.ToData()
	WriteHeader(int32(len(body)), int32(msg.seq), byte(msg.cmd), byte(msg.version), byte(msg.flag), buf)
	buf.Write(body)
}
...
func ReceiveLimitMessage(conn io.Reader, limitSize int, external bool) *Message {
	...
	length, seq, cmd, version, flag := ReadHeader(buf)
	...
	message.flag = flag
	if !message.FromData(buf) {
		log.Warningf("parse error:%d, %d %d %d %s", cmd, seq, version,
			flag, hex.EncodeToString(buf))
		return nil
	}
	return message
}
